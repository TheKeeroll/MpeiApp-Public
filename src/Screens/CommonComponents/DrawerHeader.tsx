...
const DrawerHeader: React.FC<{navigation: any, title: string}> = (props) => {
    ...
    return (
        <View style={{shadowOpacity: .4, shadowColor: '#00000040', shadowOffset: {height: 4, width: 0}, width: '100%', height: Platform.OS == 'android' ? '10%' : '7.5%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.backdrop}}>
            ...
            <Text style={{alignSelf: 'center', fontWeight: '600', textAlign: 'center',  fontSize: 22, color: colors.text}}>{props.title}</Text>
        </View>
    )
}
    ...
    return (
        <View style={{shadowOpacity: .4, shadowColor: '#00000040', shadowOffset: {height: 4, width: 0}, width: '100%', height: Platform.OS == 'android' ? '10%' : '7.5%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.backdrop}}>
            {backable &&
                ...
                }} style={{height: '100%', zIndex: 10, position: 'absolute', left: 0, justifyContent: 'center'}}>
                    <MtIcons.default size={40} color={colors.text} name={'navigate-before'} adjustsFontSizeToFit/>
                </TouchableOpacity>
            }
            
            <View style={{flex: 1, height: '100%', alignItems: 'center', justifyContent: 'center'}}>
                <Text adjustsFontSizeToFit numberOfLines={2} style={{alignSelf: 'center', fontWeight: '600', width: '80%', textAlign: 'center',  fontSize: 22, color: colors.text}}>{props.title}</Text>
            </View>
        </View>
    )
}
